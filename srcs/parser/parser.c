...
/*   Updated: 2022/11/19 22:33:04 by nfukuma          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static bool	parse_logical_ope(t_command **last_cmd, t_node **node,
				t_token_list **token);
static bool	parse_command(t_command **last_cmd, t_node **node,
				t_token_list **token);
static bool	parse_redirect_process(t_node *node, t_token_list **token);
...
static bool	parse_redirect_process(t_node *node, t_token_list **token)
{
	...
	if ((*token)->type == IO_NUMBER && ft_atoi_limit((*token)->comp,
			&redirect->fd_io) == false)
	{
